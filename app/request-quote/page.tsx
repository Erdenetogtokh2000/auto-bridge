...
export default async function RequestQuotePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const rawListing = params.listingUrl;
  const listingUrl = Array.isArray(rawListing) ? rawListing[0] ?? "" : rawListing ?? "";
  const initial = { market, vehiclePrice: numberParam(params, "vehiclePrice", 0), vehicleCurrency, purchaseFeeMnt: numberParam(params, "purchaseFeeMnt", 0), inlandTransportMnt: numberParam(params, "inlandTransportMnt", 0), oceanFreightUsd: numberParam(params, "oceanFreightUsd", 0), krwMntRate: numberParam(params, "krwMntRate", 2.6), usdMntRate: numberParam(params, "usdMntRate", 3500), customsMnt: numberParam(params, "customsMnt", 0), vatMnt: numberParam(params, "vatMnt", 0), otherCostsMnt: numberParam(params, "otherCostsMnt", 0), depositMnt: numberParam(params, "depositMnt", 0), listingUrl };
