...
export function FinancingRequestManager({ requests, canDecide = true }: { requests: RequestRow[]; canDecide?: boolean }) {
  ...
  return <section className="dashboard-panel financing-directory-panel" id="requests"><div className="dashboard-section-heading"><div><span>ББСБ · САНХҮҮЖИЛТ</span><h2>Хэрэглэгчийн хүсэлтүүд</h2></div><span className="live-data-chip">ШУУД ӨГӨГДӨЛ · {rows.length}</span></div>{rows.length ? <div className="financing-request-list">{rows.map(row => <FinancingRow key={row.id} row={row} busy={busy === row.id} onUpdate={update} canDecide={canDecide} />)}</div> : <div className="quote-empty compact"><FileText/><h3>Санхүүжилтийн хүсэлт одоогоор алга</h3><p>Захиалга баталгаажсаны дараа хэрэглэгч энэ хэсгээс санхүүжилтийн хүсэлт илгээх боломжтой.</p></div>}</section>;
}
