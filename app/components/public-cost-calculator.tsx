...
export function PublicCostCalculator({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={`public-calculator ${compact ? "is-compact" : ""}`}>
    <section className="public-calculator-form">
      <div className="calculator-form-heading"><div><span>PRELIMINARY ESTIMATE</span><h2>Монголд буух өртгийг тооцоолох</h2></div><Calculator size={25} /></div>
      <label className="calculator-field calculator-url-field encar-first-field"><span>ENCAR ЭСВЭЛ CARS.COM ЗАРЫН ХОЛБООС</span><div className="calculator-input"><Link2 size={14} /><input type="url" value={listingUrl} onChange={(event) => { setListingUrl(event.target.value); setListingVehicle(null); setListingStatus("idle"); }} placeholder="https://fem.encar.com/... эсвэл https://www.cars.com/..." />{listingStatus === "loading" && <LoaderCircle className="encar-spinner" size={16} />}</div></label>
      {listingStatus === "loading" && /cars\.com/i.test(listingUrl) && <div className="encar-autofill-status loading"><LoaderCircle className="encar-spinner" size={16}/><span>Cars.com зарын мэдээллийг татаж байна. Хамгаалалттай хуудаснаас мэдээлэл авахад нэг минут орчим хугацаа шаардагдаж болно.</span></div>}
      {listingStatus === "success" && listingVehicle && <div className="encar-autofill-status success"><CheckCircle2 size={17}/><div><strong>{listingVehicle.source === "CARS_COM" ? "Cars.com" : "Encar"} зарын мэдээллийг автоматаар бөглөлөө</strong><span>{[listingVehicle.make, listingVehicle.model, listingVehicle.productionYear, listingVehicle.fuelName, listingVehicle.engineCapacityCc ? `${listingVehicle.engineCapacityCc.toLocaleString("mn-MN")} cc` : null, listingVehicle.mileageKm ? `${listingVehicle.mileageKm.toLocaleString("mn-MN")} км` : null].filter(Boolean).join(" · ")}</span></div></div>}
      ...
      <div className="calculator-fields">
        ...
        <label className="calculator-field"><span>Түлшний төрөл</span><select value={fuelClass} onChange={(event) => setFuelClass(event.target.value as VehicleFuelClass)}><option value="GASOLINE_DIESEL">Бензин / Дизель</option><option value="HYBRID_LPG">Хосолмол / LPG</option><option value="ELECTRIC">Цахилгаан</option></select></label>
      ...
      <div className="calculator-note"><Info size={15} /><span>Encar эсвэл Cars.com зарын холбоос танигдвал үйлдвэрлэсэн он, үнэ, хөдөлгүүрийн багтаамж, түлшний төрөл, гүйлтийн мэдээллийг боломжтой хүрээнд автоматаар бөглөнө. Та бөглөгдсөн утгыг гараар засаж болно. Гаалийн татварыг 5%-иар, онцгой албан татварыг үйлдвэрлэсэн он, хөдөлгүүрийн багтаамж, түлшний төрлөөр, НӨАТ-ыг гаалийн үнэ болон татваруудын нийлбэрийн 10%-иар тооцно.{automaticTaxes.luxuryExciseMnt > 0 ? ` 360 сая ₮-өөс давсан гаалийн үнийн хэсэгт ${money.format(automaticTaxes.luxuryExciseMnt)} ₮-ийн нэмэлт онцгой албан татвар тооцсон.` : ""} Эцсийн төлбөр гаалийн албан ёсны үнэлгээ болон тухайн үеийн ханшаас шалтгаалан өөрчлөгдөж болно.</span></div>
    </section>
    <aside className="calculator-summary">
      <div className="calculator-summary-heading"><span>ЗАРДЛЫН ЗАДАРГАА</span><h2>Монголд буух<br />урьдчилсан өртөг</h2></div>
