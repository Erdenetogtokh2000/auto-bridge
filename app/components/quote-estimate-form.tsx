...
type Estimate = QuoteCalculationInput & { vehicleName: string; vehicleMake: string; vehicleModel: string; productionYear: number; mileageKm: number; fuelType: string; notes: string };
...
export function QuoteEstimateForm({ quoteId, initial, requesterEmail }: { quoteId: string; initial: InitialEstimate | null; requesterEmail: string | null }) {
  ...
  const [form, setForm] = useState<Estimate>({
    ...
    productionYear: initial?.productionYear ?? 0, mileageKm: initial?.mileageKm ?? 0, fuelType: initial?.fuelType ?? "",
    ...
    vatMnt: initial?.vatMnt ?? 0, otherCostsMnt: initial?.otherCostsMnt ?? 0,
  ...
  const fields: Array<[keyof QuoteCalculationInput, string, string]> = [
    ...
    ["customsMnt", "Гаалийн татвар", "MNT"], ["vatMnt", "НӨАТ", "MNT"],
  ...
  return <div className="estimate-layout">
    <section className="dashboard-panel estimate-form-panel">
      ...
      <div className="estimate-vehicle-fields">
        ...
        <div className="estimate-field"><Label htmlFor="fuelType">Түлш</Label><Input id="fuelType" value={form.fuelType} onChange={event=>setForm({...form,fuelType:event.target.value})} placeholder="Diesel" /></div>
      ...
      <div className="estimate-fields">{fields.map(([field,label,unit])=><div className="estimate-field" key={field}><Label htmlFor={field}>{label}</Label><div className="money-input"><Input id={field} type="number" min="0" step={field.includes("Rate") ? "0.01" : "1"} value={form[field]} onChange={event=>updateNumber(field,event.target.value)}/><span>{unit}</span></div></div>)}</div>
