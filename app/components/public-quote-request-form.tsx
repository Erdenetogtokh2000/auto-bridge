...
export function PublicQuoteRequestForm({ initial }: { initial: LandedCostInput & { listingUrl: string } }) {
  ...
  return <div className="quote-request-layout">
    <section className="quote-request-form">
      ...
      <form action="/api/quotes" method="post">
        <input type="hidden" name="calculatorSubmitted" value="true" /><input type="hidden" name="market" value={initial.market} /><input type="hidden" name="vehiclePrice" value={initial.vehiclePrice} /><input type="hidden" name="vehicleCurrency" value={initial.vehicleCurrency} /><input type="hidden" name="purchaseFeeMnt" value={initial.purchaseFeeMnt} /><input type="hidden" name="inlandTransportMnt" value={initial.inlandTransportMnt} /><input type="hidden" name="oceanFreightUsd" value={initial.oceanFreightUsd} /><input type="hidden" name="krwMntRate" value={initial.krwMntRate} /><input type="hidden" name="usdMntRate" value={initial.usdMntRate} /><input type="hidden" name="customsMnt" value={initial.customsMnt} /><input type="hidden" name="vatMnt" value={initial.vatMnt} /><input type="hidden" name="otherCostsMnt" value={initial.otherCostsMnt} /><input type="hidden" name="depositMnt" value={totals.depositMnt} />
    ...
    <aside className="quote-request-summary"><div className="calculator-summary-heading"><span>ТАНЫ ТООЦОО</span><h2>Урьдчилсан нийт үнэ</h2></div><strong>{money.format(totals.totalMnt)} ₮</strong><div><span>Урьдчилгаа · суурь дүнгийн 30%</span><b>{money.format(totals.depositMnt)} ₮</b></div><div><span>Машины үлдэгдэл · 70%</span><b>{money.format(totals.balanceMnt)} ₮</b></div><a href="/calculator"><Calculator size={14} /> Тооцоог засах</a></aside>
  </div>;
}
