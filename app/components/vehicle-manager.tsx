...
export function VehicleManager({vehicles}:{vehicles:ManagedVehicle[]}){
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="vehicle-dialog"><DialogHeader><DialogTitle>{editing?"Автомашины мэдээлэл засах":"Каталогт автомашин нэмэх"}</DialogTitle><DialogDescription>Зах зээл, үзүүлэлт, үнэ, зураг болон нийтлэх төлөвийг бүртгэнэ.</DialogDescription></DialogHeader><form onSubmit={submit}><div className="vehicle-form-grid">
      {!editing&&<div className="vehicle-link-import wide"><div><Download/><span><strong>Encar эсвэл Cars.com линкээс татах</strong><small>Зарын линкийг оруулахад мэдээлэл болон зургууд автоматаар бөглөгдөнө.</small></span></div><div><Input type="url" value={importUrl} onChange={e=>setImportUrl(e.target.value)} placeholder="https://www.encar.com/... эсвэл https://www.cars.com/..."/><Button type="button" onClick={importFromUrl} disabled={importBusy||!importUrl.trim()}>{importBusy?<Loader2 className="spin"/>:<Download/>}{importBusy?"Татаж байна...":"Мэдээлэл татах"}</Button></div></div>}
      ...
      <label className="wide"><span>Эх зарын линк</span><Input type="url" value={form.listingUrl??""} onChange={e=>field("listingUrl",e.target.value)}/></label><label className="wide"><span>Зургийн линк</span><Input type="url" value={form.imageUrl??""} onChange={e=>field("imageUrl",e.target.value)} placeholder="https://..."/></label><label className="wide vehicle-image-file"><span><ImagePlus/> Зураг файл</span><input ref={fileRef} type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp"/><small>JPG, PNG, WEBP · 5MB хүртэл. Файл сонговол зургийн линкийг орлоно.</small></label><label className="wide"><span>Тайлбар</span><textarea maxLength={1200} value={form.description??""} onChange={e=>field("description",e.target.value)}/></label>
